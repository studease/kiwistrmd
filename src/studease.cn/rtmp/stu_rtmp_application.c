...
stu_rtmp_application_cleanup(stu_rtmp_application_t *app) {
	...
	stu_rtmp_apps.hooks.free_fn(app);
...
stu_int32_t
stu_rtmp_accept(stu_rtmp_connection_t *nc) {
	if (stu_rtmp_application_insert(nc) == STU_ERROR) {
		stu_log_error(0, "Failed to insert rtmp connection: application=%s, instance=%s.",
...
stu_int32_t
stu_rtmp_reject(stu_rtmp_connection_t *nc) {
	nc->conn->error = TRUE;
	return STU_ERROR;
}
...
stu_int32_t
stu_rtmp_application_insert(stu_rtmp_connection_t *nc) {
	stu_int32_t  rc;
...
stu_int32_t
stu_rtmp_application_insert_locked(stu_rtmp_connection_t *nc) {
	...
	return stu_hash_insert_locked(&inst->connections, &nc->near_id, nc);
}

void
stu_rtmp_application_remove(stu_rtmp_connection_t *nc) {
	stu_mutex_lock(&stu_rtmp_apps.lock);
...
void
stu_rtmp_application_remove_locked(stu_rtmp_connection_t *nc) {
	stu_rtmp_application_t *app;
	stu_rtmp_instance_t    *inst;
	stu_uint32_t            hk;
	...
	inst = stu_hash_find_locked(&app->instances, hk, nc->url.instance.data, nc->url.instance.len);
	if (inst) {
		stu_hash_remove_locked(&inst->connections, hk, nc->near_id.data, nc->near_id.len);
	}
}
