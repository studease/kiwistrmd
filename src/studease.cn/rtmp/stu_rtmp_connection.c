...
static stu_int32_t  stu_rtmp_add_responder(stu_rtmp_connection_t *nc, stu_rtmp_responder_t *responder);


void
stu_rtmp_connection_init(stu_rtmp_connection_t *nc, stu_connection_t *c) {
	nc->conn = c;
	...
	stu_hash_init(&nc->streams, STU_RTMP_STREAM_DEFAULT_SIZE, NULL, STU_HASH_FLAGS_LOWCASE);
 ...
 */
stu_int32_t
stu_rtmp_connect(stu_rtmp_connection_t *nc, u_char *url, size_t len, stu_rtmp_responder_t *res, ...) {
	stu_connection_t *c;
 ...
 */
stu_int32_t
stu_rtmp_call(stu_rtmp_connection_t *nc, u_char *command, size_t len, stu_rtmp_responder_t *res, ...) {
	stu_connection_t *c;
	stu_rtmp_amf_t   *ao_cmd, *ao_tran, *ao_prop, *ao_args, *ao_item;
	u_char           *pos;
	u_char            tmp[STU_RTMP_REQUEST_DEFAULT_SIZE];
...
stu_int32_t
stu_rtmp_close(stu_rtmp_connection_t *nc) {
	return STU_OK;
}

stu_int32_t
stu_rtmp_create_stream(stu_rtmp_connection_t *nc, stu_rtmp_responder_t *res) {
	stu_connection_t *c;
...
stu_int32_t
stu_rtmp_set_chunk_size(stu_rtmp_connection_t *nc, stu_int32_t size) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[4];
	...
	*(stu_uint32_t *) pos = stu_endian_32(size);
	...
	if (stu_rtmp_send_buffer(nc, STU_RTMP_CSID_PROTOCOL_CONTROL, 0, STU_RTMP_MESSAGE_TYPE_SET_CHUNK_SIZE, 0, tmp, pos - tmp) == STU_ERROR) {
...
stu_int32_t
stu_rtmp_send_abort(stu_rtmp_connection_t *nc) {
	// TODO: Send abort message.
	return STU_OK;
}

stu_int32_t
stu_rtmp_send_ack_sequence(stu_rtmp_connection_t *nc) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[4];
	...
	*(uint32_t *) pos = stu_endian_32(nc->stat.bytes_in);
	...
	if (stu_rtmp_send_buffer(nc, STU_RTMP_CSID_PROTOCOL_CONTROL, 0, STU_RTMP_MESSAGE_TYPE_ACK, 0, tmp, pos - tmp) == STU_ERROR) {
...
stu_int32_t
stu_rtmp_send_user_control(stu_rtmp_connection_t *nc, stu_uint16_t event, stu_uint32_t stream_id, stu_uint32_t buffer, stu_uint32_t timestamp) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[10];
	...
	*(uint16_t *) pos = stu_endian_16(event);
	...
	if (event == STU_RTMP_EVENT_TYPE_SET_BUFFER_LENGTH) {
		*(stu_uint32_t *) pos = stu_endian_32(buffer);
		pos += 4;
	}
	...
	if (stu_rtmp_send_buffer(nc, STU_RTMP_CSID_PROTOCOL_CONTROL, 0, STU_RTMP_MESSAGE_TYPE_USER_CONTROL, 0, tmp, pos - tmp) == STU_ERROR) {
...
stu_int32_t
stu_rtmp_set_ack_window_size(stu_rtmp_connection_t *nc, stu_uint32_t size) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[4];
	...
	*(uint32_t *) pos = stu_endian_32(size);
	...
	if (stu_rtmp_send_buffer(nc, STU_RTMP_CSID_PROTOCOL_CONTROL, 0, STU_RTMP_MESSAGE_TYPE_ACK_WINDOW_SIZE, 0, tmp, pos - tmp) == STU_ERROR) {
		stu_log_error(0, "Failed to send protocol control 0x%02X: fd=%d, ack=%d.",
...
stu_int32_t
stu_rtmp_set_peer_bandwidth(stu_rtmp_connection_t *nc, stu_uint32_t bandwidth, stu_uint8_t limit) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[5];
	...
	*pos++ = limit;
	...
	if (stu_rtmp_send_buffer(nc, STU_RTMP_CSID_PROTOCOL_CONTROL, 0, STU_RTMP_MESSAGE_TYPE_BANDWIDTH, 0, tmp, pos - tmp) == STU_ERROR) {
...
stu_int32_t
stu_rtmp_send_result(stu_rtmp_connection_t *nc, stu_double_t tran, stu_rtmp_amf_t *prop, stu_rtmp_amf_t *info) {
	stu_connection_t *c;
...
stu_int32_t
stu_rtmp_send_error(stu_rtmp_connection_t *nc, stu_double_t tran, stu_rtmp_amf_t *prop, stu_rtmp_amf_t *info) {
	stu_connection_t *c;
	stu_rtmp_amf_t   *ao_cmd, *ao_tran;
	u_char           *pos;
	u_char            tmp[STU_RTMP_REQUEST_DEFAULT_SIZE];
...
stu_int32_t
stu_rtmp_send_buffer(stu_rtmp_connection_t *nc, stu_uint32_t csid, stu_uint32_t timestamp, stu_uint8_t type, stu_uint32_t stream_id, u_char *data, size_t len) {
	stu_connection_t *c;
	u_char           *pos;
	u_char            tmp[STU_RTMP_REQUEST_DEFAULT_SIZE];
	stu_uint8_t       fmt;
...
stu_rtmp_on_create_stream(stu_rtmp_request_t *r) {
	...
	stream_id = *(stu_double_t *) r->arguments->value;

	stu_log_debug(4, "Handle command \"%s\": access=%s, url=%s.", STU_RTMP_CMD_CREATE_STREAM.data, nc->read_access.data, nc->url.data);

	if (nc->read_access.len == 0 || stu_strncasecmp(nc->read_access.data, (u_char *) "/", 1) == 0) {
		ns = stu_pcalloc(nc->conn->pool, sizeof(stu_rtmp_stream_t));
		if (ns) {
			ns->id = stream_id;
			ns->on_status = NULL;

			stu_rtmp_stream_attach(ns, nc);
	...
	ao_prop = stu_rtmp_amf_create_null(NULL);
	ao_info = stu_rtmp_get_information(&STU_RTMP_LEVEL_ERROR, &STU_RTMP_CODE_NETSTREAM_FAILED, "create stream failed");
...
void
stu_rtmp_close_connection(stu_rtmp_connection_t *nc) {
	stu_hash_destroy(&nc->commands, NULL);
	stu_hash_destroy(&nc->streams, NULL);
...
static stu_int32_t
stu_rtmp_add_responder(stu_rtmp_connection_t *nc, stu_rtmp_responder_t *res) {
	stu_connection_t *c;
	u_char            tmp[10];
	stu_str_t         key;
